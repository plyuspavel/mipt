Файловая система. Популярные форматы файлов.
############################################

:summary: Файловая система. Популярные форматы файлов.
:status: published
...
| Она возвращает так называемый поток - интерфейс взаимодействия с
  содержимым файла.
| Функция open принимает первым аргументом полное имя файла (с путем,
...
| По умолчанию файл открывается в режиме ‘r’ - для чтения в текстовом
  формате.
...
бинарном режиме, для более явного управления всеми спец.символами
навроде переноса строк, табуляций.
...
| \* метод read(n) позволяет прочитать следкющие n символов файла.
  ...
  Таким образом, если выполнять read(n) несколько раз подряд, мы будем
  получать не первые n символов, а каждый раз новые, n символов.

::

   Если n явно не указать, то считается весь файл целиком (указатель окажется в самом конце файла). Для исспользования метода read, файл должен бюыть открыт в режиме для чтения  
...
-  если файл был открыт в режиме для записи, можно воспользоваться
   методом write(buffer) - записывает содержимое буффера в файл. Работа
   ...
   **Примечание:** при записи содержимого в файл, не гарантируется, что
   ...
   буферизируются кусками фиксированного размера. Этот размер
...
Вам необходимо распаковать этот архив (средставми языка python), и затем
